...
export function useAttentionItems(
  windowSec = DEFAULT_TRAFFIC_WINDOW,
  { background = false }: AttentionOptions = {},
) {
  ...
  const items = useMemo<AttentionItem[]>(() => {
    ...
    const erroring = traffic.hasRate
      ? traffic.channels
          ...
          .map((c) => {
            ...
            const sinceStart =
              cum && cum.errorPct > 0 ? ` · ${formatPct(cum.errorPct)} since the engine started` : ""
            return {
              channel: c.channel,
              share: c.errorPct ?? 0,
              detail: `${formatPct(c.errorPct)} of ${(c.ok + c.failed).toLocaleString()} requests failed in the last ${windowLabel}${sinceStart}`,
            }
          })
      : metrics.channels
          ...
          .map((c) => ({
            channel: c.channel,
            share: c.errorPct,
            detail: `${formatPct(c.errorPct)} of processed requests failed since the engine started (${c.failed.toLocaleString()}) · a second sample will narrow this to the last ${windowLabel}`,
          }))
    ...
    const list: AttentionItem[] = [
      ...
      ...openBreakers(breakers).map(
        ({ key, state }): AttentionItem => ({
          key: `brk-${key}`,
          severity: 6,
          tone: "warning",
          kind: "breaker",
...
export function useNavCounts() {
  const { items, hasCron } = useAttentionItems(DEFAULT_TRAFFIC_WINDOW, { background: true })
  const { data: exhausted } = useTraceDlq({ limit: 1, exhausted: true }, { refetchInterval: 60_000 })
  const { data: schedules } = useCronStatus({ enabled: hasCron, refetchInterval: 60_000 })
  const pending = (schedules ?? []).reduce((sum, s) => sum + (s.pending ?? 0), 0)
  return {
    alerts: items.length,
    dlq: exhausted?.total ?? 0,
    breakers: items.filter((i) => i.kind === "breaker").length,
    schedules: pending,
  }
}
